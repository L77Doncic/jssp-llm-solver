...
| 1 | 参考求解器与数据构建 | ③ | OR-Tools CP-SAT 封装；实例生成器；SFT 数据集 + splits | 🔄 进行中（服务器） |
| 2 | 输入输出表示 | ② | TAI 文本模板；JSON 排程格式；解析器 + 验证器 + 测试 | 待启动 |
| 3 | SFT | ④ | LoRA 微调脚本与首个检查点 | 待启动 |
| 4 | FOARL | ④ | GRPO 训练脚本；奖励实现（可行+最优） | 待启动 |
| 5 | 评估与泛化 | ⑤ | 自动评估流水线；基线对比；泛化实验 | 待启动 |
