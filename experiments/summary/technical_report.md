...
工程基建：107 个单元测试全绿（约束验证、makespan 计算、解析器、奖励函数）；git 版本管理（26 commits）；无人值守训练监控；CLAUDE.md 维护手册沉淀 18 条工程经验。
...
### 5.6 消融：TAI 启发式特征（完整对照，60 实例 6×6）

| 训练注入 | 推理注入 | 可行率 | 平均 gap | 最优 gap |
|---|---|---|---|---|
| 无（纯 TAI） | 无 | 100.0%* | 41.54% | 10.14% |
| 无（纯 SFT 模型） | 无 | 78.3% | 29.61% | 6.81% |
| SPT 提示 | 无 | 8.3% | 17.29% | 7.60% |
| SPT 提示 | SPT 提示 | 8.3% | 10.70% | 5.32% |

（*BoN 消融口径：至少 1 个可行采样。前三行同口径对比。）

结论（重要负结果，含失败模式裁决实验）：
1. **SPT 提示训练注入破坏可行性**：无论推理是否注入，可行率均从 78.3% 跌至 ~8.3%（transformers 与 vLLM 双引擎、三个独立评估文件交叉确认，非采样噪声）。
2. **破坏机制 = 输出格式纪律崩溃**（失败模式分析，30 实例 × 8 采样分类）：hint 模型的失败以「JSON 格式错误」为主（43.3% vs 纯 SFT 8.8%），而非约束违规——模型围绕目标数字组织输出，导致 JSON 结构破碎。纯 SFT 的失败则以「机器冲突」（57.1%）为主，两类模型失败模式显著不同。
3. **但对可行实例质量有增益**：gap 从 29.61% 改善到 10.70%（最优 gap 6.81% → 5.32%）——目标锚点帮助了数字精度，代价是输出结构。
4. 工程教训（vLLM 多 LoRA）：同一引擎加载多个 adapter 时 LoRA int_id 必须唯一，否则缓存冲突导致结果污染（本实验首轮数据即因此作废重跑）。
5. 说明 JSSP 上廉价规则特征的注入方式需要精细设计（特征类型、提示格式、训练与推理一致性），该负结果已列入后续研究方向。
