...
结论（重要负结果，三组交叉验证）：
1. **SPT 提示训练注入破坏可行性**：无论推理是否注入，可行率均从 78.3% 跌至 8.3%——hint 标量在训练中引入分布偏移，模型过度依赖目标锚点而牺牲约束结构，与论文报告的启发式特征收益相反。
2. **但对可行实例质量有增益**：gap 从 29.61% 改善到 10.70%（最优 gap 6.81% → 5.32%）——目标锚点确实帮助了质量。
3. 说明 JSSP 上廉价规则特征的注入方式需要精细设计（特征类型、提示格式、训练与推理一致性），该负结果已列入后续研究方向。
