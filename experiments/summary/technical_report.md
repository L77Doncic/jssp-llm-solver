...
**考核项目技术报告** ｜ 2026-08-13 ｜ 模型：Qwen2.5-7B-Instruct + LoRA ｜ 算力：RTX 5090 32GB ×1
...
```
实例(JSON) ──→ TAI 文本模板 ──→ LLM(Qwen2.5-7B + LoRA) ──→ JSON 排程 ──→ 容错解析 ──→ 三约束验证器 ──→ makespan/gap
                   ↑                                    ↑
             (启发式特征, 可插拔)              Best-of-N 采样（N=8）
```
...
- **代码仓库**：GitHub `jssp-llm-solver`（代码/配置/文档/实验结果全量入库）
- **数据**：42K 监督样本 + 123 公开基准（服务器数据盘，可由脚本重建）
- **模型产物**：`sft_qwen7b/final`（SFT）、`foarl_qwen7b/epoch2`（FOARL 最终模型）
- **测试**：107 pytest 全绿
- **维护手册**：CLAUDE.md（18 条工程经验）
